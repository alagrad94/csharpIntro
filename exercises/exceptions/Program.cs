using System;
using System.Collections.Generic;

namespace exceptions {

  class Program {

    static void Main() {

      // Create a few contacts
        Contact bob = new Contact("Bob", "Smith", "bob.smith@email.com", "100 Some Ln, Testville, TN 11111");

        Contact sue = new Contact("Sue", "Jones", "sue.jones@email.com", "322 Hard Way, Testville, TN 11111");

        Contact juan = new Contact("Juan", "Lopez", "juan.lopez@email.com", "888 Easy St, Testville, TN 11111");

        // Create an AddressBook and add some contacts to it
        AddressBook addressBook = new AddressBook();
        addressBook.AddContact(bob);
        addressBook.AddContact(sue);
        addressBook.AddContact(juan);

        addressBook.PrintContacts();

        // Try to addd a contact a second time
        addressBook.AddContact(sue);

        // Create a list of emails that match our Contacts
        List<string> emails = new List<string>() {
            "sue.jones@email.com",
            "juan.lopez@email.com",
            "bob.smith@email.com",
        };

        // Insert an email that does NOT match a Contact
        emails.Insert(1, "not.in.addressbook@email.com");


        //  Search the AddressBook by email and print the information about each Contact
        foreach (string email in emails) {

          try {
            Contact contact = addressBook.GetByEmail(email);
            Console.WriteLine("----------------------------");
            Console.WriteLine($"Name: {contact.FullName}");
            Console.WriteLine($"Email: {contact.Email}");
            Console.WriteLine($"Address: {contact.Address}");
          }
          catch (System.NullReferenceException) {
           Console.WriteLine($"{email} does not match a contact in your address book.");
          }

        }
    }
  }
}
